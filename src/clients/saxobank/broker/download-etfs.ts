...
export async function downloadSaxoBankETFs(app: SaxoBankApplication, outputPath: string): Promise<void> {
  ...
  await writeTSFile({
    filePath: path.join(outputPath, 'etf.ts'),
    constants,
  })

  // deno-lint-ignore no-console
  console.log(
    `Included ${[...foundStocks.values()].reduce((sum, count) => sum + count, 0)} ETFs:`,
