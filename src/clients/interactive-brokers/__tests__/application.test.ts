...
import { InteractiveBrokersApplication } from '../application.ts'
...
describe(InteractiveBrokersApplication.name, () => {
  test('Getting brokerage session status', async () => {
    ...
    await using app = new InteractiveBrokersApplication({ type: 'Live' })

    const status = await app.iserver.auth.status.post()
  ...
  test('Finding all available contracts', async () => {
    ...
    const contracts = await app.trsrv.allConids.get({
  ...
  test('Finding contracts based on company name or symbol', async () => {
    ...
    const aaplBySymbol = await app.iserver.secdef.search.post({
      symbol: 'AAPL',
      secType: 'STK',
    })
    ...
    const aaplByName = await app.iserver.secdef.search.post({
      symbol: 'APPLE INC',
      name: true,
      secType: 'STK',
    })
  ...
  test('Getting contract information', async ({ step }) => {
    ...
    for (const [ticker, contractId] of Object.entries(CONTRACTS)) {
      await step(ticker, async () => {
        const contract = await app.iserver.contract.infoAndRules.get({
          conid: contractId,
        })
  ...
  test('Getting security information for contracts', async ({ step }) => {
    ...
    for (const [ticker, contractId] of Object.entries(CONTRACTS)) {
      await step(ticker, async () => {
        const info = await app.iserver.secdef.info.get({
          conid: contractId,
        })
  ...
  test('Getting trading schedule for a symbol', async () => {
    ...
    const aaplSchedule = await app.trsrv.secdef.schedule.get({
      assetClass: 'STK',
      symbol: 'AAPL',
  ...
  test('Finding contract ids for forex currency pairs', async () => {
    ...
    const contracts = await app.iserver.currency.pairs.get({
      currency: ['USD', 'DKK'],
    })
  ...
  test('Finding the current exchange rate between two currencies', async () => {
    ...
    const eurusdRate = await app.iserver.exchangeRate.get({
      source: 'EUR',
      target: 'USD',
    })
  ...
  test('Getting a market data snapshot', async () => {
    ...
    const accounts = await app.iserver.accounts.get()
    ...
    while (++iteration <= 100) {
      const snapshot = await app.iserver.marketData.snapshot.get({
  ...
  test('Getting account funds (balances, margins, etc.)', async () => {
    ...
    const accountId = Environment['IB_ACCOUNT_ID']
    if (accountId === undefined) {
      throw new Error('Missing IB_ACCOUNT_ID in .env')
    }
    ...
    const summary = await app.iserver.account.summary.get({
      accountId,
    })
    debug('summary', summary)
    expect(summary).toBeDefined()

    const availableFunds = await app.iserver.account.summary.availableFunds.get({
      accountId,
    })
    debug('available funds', availableFunds)
    expect(availableFunds).toBeDefined()

    const balances = await app.iserver.account.summary.balances.get({
      accountId,
    })
    debug('balances', balances)
    expect(balances).toBeDefined()

    const margins = await app.iserver.account.summary.margins.get({
      accountId,
    })
    debug('margins', margins)
    expect(margins).toBeDefined()

    const marketValue = await app.iserver.account.summary.marketValue.get({
      accountId,
    })
  ...
  test('Simulation', async () => {
    ...
    await using app = new InteractiveBrokersApplication({ type: 'Paper' })
