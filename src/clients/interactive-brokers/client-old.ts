...
const debug = {
  get: Debug('ib-client:get'),
  post: Debug('ib-client:post'),
  delete: Debug('ib-client:delete'),
}
...
export class InteractiveBrokersClient<Options extends InteractiveBrokersClientOptions = InteractiveBrokersClientOptions>
  ...
  constructor(options: Options) {
    const consumerKey = Environment.get('IB_CONSUMER_KEY')
    const accessToken = Environment.get('IB_ACCESS_TOKEN')
    const accessTokenSecret = Environment.get('IB_ACCESS_TOKEN_SECRET')
    const diffieHellmanPrime = Environment.get('IB_DIFFIE_HELLMAN_PRIME')
  ...
  async get<T = unknown>(options: {
    ...
    const url = urlJoin(this.#env.baseUrl, options.path)
    if (options.searchParams !== undefined) {
      url.search = buildURLSearchParams(options.searchParams).toString()
    }

    let liveSessionToken: undefined | LiveSessionToken = undefined

    const mergedSignal = mergeAbortSignals(this.#controller.signal, options.signal)
